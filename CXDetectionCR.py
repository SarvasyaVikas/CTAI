...
def load_CX_attributes():
	...
	for row in csvreader:
		CXcode = row[0]
		CXcodes.append(CXcode)
		CXattr = row[1:5:]
		CXrows.append(CXattr)
	
	return (CXcodes, CXrows)

def load_CX_images(CXattr):
	rawlst = []
	
	for code in range(len(CXattr)):
...
(CXcodes, CXattr) = load_CX_attributes()

rawlst = load_CX_images(CXcodes)

(trainData, testData, trainAttr, testAttr, trainFiles, testFiles) = train_test_split(rawlst, CXattr, CXcodes, test_size = 0.10, random_state = 42)
...
print(classification_report(testAttr.argmax(axis = 1), preds.argmax(axis = 1), target_names = ["CXLTX", "CXLTY", "CXRBX", "CXRBY"]))

model.save("CXDetector", save_format = "h5")
...
plt.title("CX Regression Loss on Training Set")
plt.xlabel("Epoch #")
plt.ylabel("Loss")
plt.legend(loc="lower left")
plt.savefig("CX_Loss_Diagram.png")
